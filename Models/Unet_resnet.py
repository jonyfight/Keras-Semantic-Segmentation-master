...
def Unet_resnet(classes, input_height=224, input_width=224):
    inputs = Input(shape=(input_height, input_width, 3))

    conv1_1 = Conv2D(64 , 7, strides=(2,2), padding='same', use_bias=False, kernel_initializer='he_normal')(inputs)
    ...
    outputHeight = Model(inputs, conv11).output_shape[1]
    outputWidth = Model(inputs, conv11).output_shape[2]
    conv11 = (Reshape((outputHeight * outputWidth, classes)))(conv11)
    activation = Activation('softmax', name='Classification')(conv11)  # sigmoid
